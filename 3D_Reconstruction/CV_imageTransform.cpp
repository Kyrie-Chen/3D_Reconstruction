...
void GetContoursPic(const char* pSrcFileName, const char* pDstFileName)
{
	...
	vector<vector<Point> > contours;	//保存点的集合的向量，每一组点集为一个轮廓---
	vector<Rect> boundRect(contours.size());	//即轮廓数---
	...
	for (int i = 0; i < contours.size(); i++)
	{
		...
		//求得包含点集最小面积的矩形（可有偏转角度），返回2D盒子CvBox2D类型（包括矩形中心、长宽和夹角）---
		...
		//对得到的轮廓白色填充一下，得到掩模binImg---
		drawContours(binImg, contours, -1, Scalar(255), CV_FILLED);
		//imshow("binImg", binImg);

		//抠图到RoiSrcImg（把binImg和srcImg重叠传给RoiSrcImg---）
...
void main()
{
	GetContoursPic("E:\\Doc_OpenCV\\image_correct.jpg", "E:\\Doc_OpenCV\\FinalImage.jpg");
	waitKey();
}